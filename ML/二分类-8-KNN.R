...
# 数据预处理配方
datarecipe_knn <- recipe(AHD ~ ., traindata) %>%
...
# 预测
predresult <- Heart %>%
  bind_cols(predict(final_knn, new_data = Heart, type = "prob"))%>%
  mutate(
    .pred_class = factor(
...
# 变量重要性
set.seed(42)
vip_knn <- DALEX::model_parts(
  explainer_knn,
  type = "ratio"
)
plot(vip_knn)
plot(vip_knn, show_boxplots = FALSE)

# 变量偏依赖图
# 连续型变量
set.seed(42)
pdpc_knn <- DALEX::model_profile(
  explainer_knn,
...
fastshap::force_plot(
  ...
  baseline = mean(predtrain_knn$prediction$.pred_Yes),
  display = "viewer"
)
# 所有分类变量的shap图示
...
for (i in lcN) {
  ...
  predtest_i_knn <- eval4cls2(
    model = i_knn, 
    dataset = testdata, 
    yname = "AHD", 
    modelname = "KNN", 
    datasetname = paste("testdata", i, sep = "-"),
    cutoff = predtrain_i_knn$diycutoff,
    positivelevel = yourpositivelevel,
    negativelevel = yournegativelevel
  )
  
  predi <- bind_rows(predtrain_i_knn$metrics, predtest_i_knn$metrics)
